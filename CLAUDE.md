...
- Every route except `/api/health`, `/api/auth/register` and
  `/api/auth/login` requires `Authorization: Bearer <token>` — get one via
  `POST /api/auth/register` or `/login` first when hitting the API with curl.
...
- `KombiModul` (M:N with `Modul` via the `kombimodul_module` table) always
  requires its own `studiengang_id` (unlike `Modul`, which can have zero
  Studiengänge = "Sonstiges"). Its grade is the arithmetic mean of its source
  modules' final grades, and only resolves once *every* source module has a
  numeric final grade.
- `Modul` <-> `Studiengang` is many-to-many (`modul_studiengang` table, same
  shape as `kombimodul_module`): a module can be credited in several
  programs at once, each independently. `stats.py`'s per-Studiengang
  "overall" bucket still counts each `Modul`/`KombiModul` row exactly
  once (`Modul.query.filter_by(user_id=...).all()`, not a join through the
  Studiengang side) — a module credited twice does **not** double its real
  credits in the "overall" total, only in each Studiengang's own bucket.
...
### Auth: every row is owned, every route is scoped

`User` (email/password via `werkzeug.security`) owns `Studiengang`, `Modul`
and `KombiModul` directly via a non-nullable `user_id` FK on each (not just
transitively through Studiengang, since a `Modul` can have zero
Studiengänge). `app/auth.py`'s `login_required` decorator populates
`g.current_user`; every route in `routes/*.py` (except `/health` and
register/login) carries it and filters/validates every query by
`user_id == g.current_user.id`. `app/utils.py:get_owned(model, id, user_id)`
is the standard single-row fetch-or-None-for-404 helper — use it (or the
transitive equivalents in `routes/submissions.py` that walk up to the
owning `Modul`) for any new by-ID route rather than a bare
`db.session.get()`, or the row-not-found-vs-belongs-to-someone-else
distinction silently disappears. Auth itself is a stateless bearer token
(`itsdangerous`, 30-day expiry, `SECRET_KEY` env var) — no session table, no
server-side logout (the frontend just discards the token).

**SQLite foreign keys are off by default** and Flask-SQLAlchemy doesn't
enable them — `models.py` turns them on globally via an
`event.listens_for(Engine, "connect")` hook so the `ondelete=` behavior
declared on every FK actually fires. Deleting a `Studiengang` also needs an
explicit ORM `cascade="all, delete-orphan"` on `Studiengang.kombi_module`
(not just `ondelete="CASCADE"`) because `KombiModul.studiengang_id` is
NOT NULL — without it SQLAlchemy tries to null the FK out before delete and
raises an IntegrityError. Keep this pattern (ORM cascade for NOT NULL child
FKs, DB `ondelete` as the general-purpose net) when adding new owned
relationships.
...
Auth follows the same client-fetched pattern: `lib/AuthContext.tsx`
(`AuthProvider`/`useAuth()`) holds the token in `localStorage` and the
current `User` in React state; `components/AuthGate.tsx` (mounted once in
`layout.tsx`) redirects unauthenticated visitors to `/login` and
authenticated ones away from `/login`/`/signup`, and is also what decides
whether `<Nav>` renders. `lib/api.ts`'s `request()` attaches the token to
every call automatically and, on a `401` *that was sent with a token*
(session expiry, not a failed login attempt), clears it and bounces to
`/login` — don't add per-page 401 handling, it's centralized there.
Destructive actions use `useConfirm()` (`components/ConfirmDialog.tsx`, a
promise-based replacement for `window.confirm`) instead of the browser
dialog — reuse it rather than reintroducing `confirm()`.
...
`User` 1—N `Studiengang`/`Modul`/`KombiModul` (each owned directly).
`Studiengang` M—N `Modul` (zero Studiengänge = "Sonstiges") and 1—N
